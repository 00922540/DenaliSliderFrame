...
namespace Keysight.ModularInstruments
{
    ...
    public class CurrentSourceAdjustCal: WyvernCalBase
    {
        ...
        //save wyvern cal data to file and save file to flash
        public void WriteCalValueToLogFile(int VtermValue)
        {
            ...
            CalFileName = String.Format("WyvernCalData_{0}.txt", whichWyvern);
        ...
        public int GetVtermValueFormCalData()
        {
            ...
            string logfile = System.IO.Path.Combine(pathDir, String.Format("WyvernCalData_{0}.txt", whichWyvern));
        ...
        public void LoadCalDataToModule()
        {
            ...
            {
                MessageBox.Show("Don't find " + logfile, "提示");
            }
        ...
        private Dictionary<string, double> MeasureBitWeights()
        {
            ...
            foreach (char AB in new char[] { 'A', 'B' })
            {
                ...
                foreach (string SwEl in SwitchingElements[AB])
                {
                    ...
                    SetPatternInForceMux(SwEl, 1); //by zhangfan for banjo
                    VHigh = measVolt(1);
