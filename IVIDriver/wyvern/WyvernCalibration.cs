...
namespace Keysight.ModularInstruments
{
     ...
     * 
     * */
    public class CurrentSourceAdjustCal
    {
        ...
        private WyvernRegisterSet mRegiserset;
        private string whichWyvern = "";
        private double mRmsMismatch = 0;
        private string mSavePath = "";
        private MeasureMarkerValueDelegation mMeasureMarkerValueMethod = null;
        private PrintTestResultDelegation mPrintTestResultMethod = null;
        ...
        private void WriteWyvernRegisterField(string regName, string BFName, int value)
        {
            getBitField(getRegister(regName), BFName).Write(value);
        }

        private int ReadWyvernRegisterField(string regName, string BFName)
        {
            getRegister(regName).Read();
            return getBitField(getRegister(regName), BFName).Value;
        }

        private void WriteWyvernRegister(string regName, int value)
        {
            getRegister(regName).Write(value);
        }

        private int ReadWyvernRegister(string regName)
        {
            return getRegister(regName).Read();
        }

        private IRegister getRegister(string regName)
        {
        ...
        private IBitField getBitField(IRegister reg, string BFName)
        {
        ...
        private double ReadMarkerAmplitude()
        {
        ...
        private double measVolt(int numAvgs)
        {
            double amplitude = 0;
            double volts = 0;
        ...
        private double ConvertdBmtoVrms(double amplitude)
        {
            double volts;
