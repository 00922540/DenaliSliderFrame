    ...
    public class SwitchDriverOffsetCal: WyvernCalBase
    {
        ...
        public override void Execute()
        {
                ...
                foreach (char psm in new char[] { 'M', 'P', 'S' }) //M,P,S in perl code
                {
                    for (int i = midState[new Tuple<char, char>('A', psm)].Length - 1; i >= 0; --i)
                    {
                        if (psm == 'M' || i >= minLsb)
                        {
                            ...
                            {
                                //measure at minimum and maximum
                                //mPrintTestResultMethod("Mux Registers before finding adj gain");
                                //LogWyvernRegisters(DebugRegisters, false);
                                writeAdj(name, adjMin);
                                measOffset(name, adjMin, dutyErr);

                                //mPrintTestResultMethod("Mux Registers after writing minimum code to " + name);
                                //LogWyvernRegisters(DebugRegisters, false);

                                writeAdj(name, adjMax);
                                measOffset(name, adjMax, dutyErr);
                                //mPrintTestResultMethod("Mux Registers after writing maximum code to " + name);
                                //LogWyvernRegisters(DebugRegisters, false);
                            ...
                            while (iter < niter)        //cycle through iterations
                            {
                                ...
                                code = newCode;
                                writeAdj(name, code);
                                //LogWyvernRegisters(DebugFileName);
                                measOffset(name, code, dutyErr);
                            }
                            //LogWyvernRegisters(DebugRegisters, false);
            ...
            foreach (string regname in saveRegs.Keys)
            {
                ...
                //DUT.Refresh();
                //DUT.WriteRegisterField(Module, whichWyvern, regname, saveRegs[regname]);
        ...
        public void SaveCalDataToModule()
        {
            ...
            string logfile = System.IO.Path.Combine(pathDir, String.Format("WyvernSwitchCalData_{0}_{1}.txt", mSlugSerialNumber, whichWyvern));
            SaveCalDataToModule(logfile);
            ...
            foreach (char dac in new char[] { 'A', 'B' })
            {
                ...
                foreach (char psm in new char[] { 'P', 'S', 'M' })
                {
                    ...
                    for (int i = iStart; i <= iStop; i++)
                    {
                        ...
                        //give this a double take
                        //DUT.Refresh();
                        //todo
                        //ReadWyvernRegisterField( regName, fieldName);
                        //DUT.Refresh();
                        //todo

                        //testestestes

                        //int currentVal = (int)ReadWyvernRegisterField( regName, fieldName);
